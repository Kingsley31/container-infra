...
# DigitalOcean Droplet PTR Record Setter
# Sets PTR record by renaming droplet with specified FQDN and updates local hostname
...
if [[ -z "${DO_API_TOKEN:-}" ]]; then
    ...
    echo "Or run: DO_API_TOKEN=your_token $0 <droplet-id> <fqdn>"
    exit 1
fi

# Check if both arguments are provided
if [[ $# -ne 2 ]]; then
    print_status "Error: Invalid number of arguments" "$RED"
    usage
fi
...
# Validate FQDN format (basic validation)
...
# Get current droplet name
current_name=$(get_droplet_name)
print_status "Current droplet name: $current_name" "$GREEN"

# Set local hostname first
print_status "Updating local machine hostname..." "$BLUE"
set_local_hostname "$FQDN"

# Rename droplet to set PTR record
print_status "Setting PTR record by renaming droplet to: $FQDN" "$YELLOW"
if rename_droplet; then
    print_status "✓ Rename request accepted by DigitalOcean API" "$GREEN"
else
    print_status "✗ Failed to rename droplet via API" "$RED"
    print_status "Please check your API token permissions and try again" "$RED"
    exit 1
fi

# Verify the droplet was actually renamed
if verify_droplet_rename "$FQDN"; then
    ...
    print_status "The rename may still be in progress or there may be API delays" "$YELLOW"
fi

print_status "Local hostname has been set to: $(hostname)" "$GREEN"
print_status "Droplet PTR record should now point to: $FQDN" "$GREEN"
print_status "Note: DNS changes may take additional time to propagate" "$YELLOW"

print_status "Operation completed!" "$GREEN"